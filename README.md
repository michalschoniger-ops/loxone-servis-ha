...
Verze 3.0.18 opravuje živé Milesight náhledy podle read-backu všech deseti kanálů: poškozené zahřívací rámce se nepropustí do UI, start jednotlivých náhledů má řízený rozestup, detail uvolní náhledové RTSP relace a nedostupný obraz skončí pravdivým stavem místo nekonečného spinneru. Detailní schopnosti, třetí MJPEG stream a VCA HTTP notifikace se dál ověřují pouze přes NVR Channel Access a po každém zápisu se provádí read-back. Součástí je Evora Smart Menu 3.0.10 s originální značkou Milesight a Windows Config Launcher 3.0.0.4 s bezokenním watchdogem i restartem přes `wscript.exe`. Hodinový Microsoft Graph import zůstává pouze směrem do Hubu a zápis zpět je vypnutý; dosavadní data, vlastní názvy kamer, DPAPI párování a šifrovaná připojení zůstávají zachovaná.
