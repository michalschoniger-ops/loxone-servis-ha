...
Verze 3.0.54 animuje úvodní desktopovou obrazovku krátkým složením společného motivu, světelným přejezdem přes diagonálu a plynulým nástupem textu; při omezeném pohybu zůstává statická. Opravuje také parkovací bránu: web posílá platný JSON požadavek, Hub bezpečně následuje nejvýše jedno HTTPS přesměrování mezi důvěryhodnými servery Loxone Cloud a úzce opraví jejich chybné UTF-8 překódování českého znaku jen u cesty přesně odpovídající původnímu povelu. Odděleně šifrované přihlášení zůstává pouze na serveru. Osobní párování, plné a technické macOS Menu 3.0.35, Windows Menu 3.0.31 a databázové schéma 24 zůstávají zachované.
