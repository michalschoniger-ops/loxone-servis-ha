...
- Fotografie je vlevo, aktivní kontakty vpravo a zelený nebo oranžový prstenec odpovídá provozní době v časové zóně Europe/Prague.
- Zachovává Launcher 3.0.0.16, všech 18 řádků školení, databázové schéma 27, stávající `/data`, šifrované přístupy a vratný rollback na předchozí zdroj.
...
- Payload před výměnou ověřuje SHA-256, cílový slug i verzi a živou 3.0.66 zachová jako rollback.

## 3.0.66.1

- Vratně připravuje Evora Smart Hub 3.0.66 s přijetím aktuální úspěšné účetní odpovědi LOXONE Portálu bez obecného příznaku `valid`.
- Payload před výměnou ověřuje SHA-256, cílový slug i verzi a živou 3.0.65 zachová jako rollback.
...
- Payload před výměnou ověřuje SHA-256, cílový slug i verzi a původní 3.0.63 zachová jako rollback.
...
- Payload před výměnou ověřuje SHA-256, cílový slug i verzi a původní 3.0.61 zachová jako rollback.
...
- Zachovává Hub 3.0.55 jako vratný zdroj, databázové schéma 25, živě ověřené otevření i zavření brány, úvodní animaci, `/data` a uložené přístupy.
