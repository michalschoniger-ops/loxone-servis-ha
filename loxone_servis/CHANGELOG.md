...
- Miniserverové akce Hubu, plného macOS Menu 3.0.17 a Windows Menu 3.0.21 přidávají bezpečné otevření webového rozhraní přes čerstvě rozlišenou adresu. URL nesmí obsahovat přihlašovací údaje, dotaz ani fragment a klient funkci povolí jen tehdy, když ji živý Hub výslovně deklaruje.
- Windows Menu 3.0.21 slučuje viditelný Config launcher pod jedinou značkovou tray ikonu; na nejvyšší úrovni nechává pouze přímé hledání a LOXONE. Má barevné ikony, vestavěné obrázky Miniserver, Compact a Go, přímé webové tlačítko a nadále ověřuje automatickou aktualizaci pomocí podepsaného manifestu a SHA-256.
- macOS Menu 3.0.17 vytváří stovky položek Miniserverů a rozsáhlý seznam ticketů líně až při otevření konkrétní složky nebo při hledání; dlouhé víceřádkové názvy ticketů zkrátí do jednoho řádku. `Systémy` používají hotový lokální snapshot, zatímco fleet, stav Windows Launcheru a Intranet se obnovují mimo otevření menu. Hledací pole si po kliknutí explicitně převezme klávesový field editor.
...
- Přidává nativní bezkonzolové Evora Smart Menu 3.0.21 pro Windows. Tray aplikace slučuje hledání a LOXONE/Config akce pod jedinou ikonou, používá stávající párování Hubu, ukládá tajemství přes DPAPI a přijímá jen HMAC/SHA-256 ověřený aktualizační balíček s rollbackem.
