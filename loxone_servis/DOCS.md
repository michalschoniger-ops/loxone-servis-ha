...
Hlavní navigace obsahuje pole **Hledat všude…**. Vyhledává bez ohledu na diakritiku v celé aplikaci: ve všech obrazovkách, podsekcích a dostupných datových záznamech; výsledek otevře přímo. Stejné globální hledání nabízí Evora Smart Menu 3.0.17 pro celé nativní macOS menu a všechny jeho vnořené položky; po přímém kliknutí pole udrží klávesový fokus. Shodný Miniserver se v globálním i samostatném hledání zobrazí rovnou jako jednotlivý výsledek a jediná shoda rovnou jako úplný detail s akcemi, nikoli jako složkový mezikrok; složky zůstávají jen při běžném procházení.
