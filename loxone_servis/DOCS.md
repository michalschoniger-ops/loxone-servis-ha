...
Hub publikuje pouze kanál 7 `Parkoviště a brána`. V macOS Menu je kamera přímo v kořeni a položka Milesight se nezobrazuje. Hub z přímého RTSP zdroje vytváří jedinou autentizovanou H.264/fMP4 HLS relaci; RTSP adresa ani přístupy se do browseru nebo Menu neposílají. Připnutý `init.mp4` umožní bezpečný start i klientovi otevřenému dlouho po vzniku relace. Playlist obsahuje jen souvislé fragmenty, které Hub už celé načetl a drží v omezené paměti; právě vznikající fragment se dokončí na pozadí a zveřejní teprve potom. Browser použije native HLS nebo připnutý hls.js fallback a macOS Menu jediný AVFoundation přehrávač. Stav `ŽIVĚ` vyžaduje opakovaný postup dekódovaného času; při zastavení zůstane poslední snapshot, zobrazí se chyba a spustí omezený reconnect. Aktivní přehrávání nepoužívá MJPEG.
