...
Hlavní navigace obsahuje pole **Hledat všude…**. Vyhledává bez ohledu na diakritiku v celé aplikaci: ve všech obrazovkách, podsekcích a dostupných datových záznamech; výsledek otevře přímo. Evora Smart Menu 3.0.31 používá skutečné systémové macOS `NSMenu`, takže otevření, zavření, umístění a podnabídky řídí AppKit bez vlastního panelu a bez zneaktivnění dosavadního okna při pouhém otevření. Nabídka je tmavá už před prvním vykreslením; celý nativní HUD materiál se míchá jen s vlastním tmavým podkladem menu, vlastní karty jsou ztmavené, barevné má pouze ikony a nepoužívá opakované přebarvování ani pulzování celé stavové ikony. Zachovává standardní řádky zleva doprava; AppKit otevře kaskádu na straně s dostupným místem. Klik do hledání obnoví jeho field editor bez aktivace celé aplikace. Shodný Miniserver se zobrazí rovnou jako jednotlivý výsledek s jednou informační kartou a klasickými akčními řádky, zatímco nesouvisející kořenové položky se skryjí; složky zůstávají jen při běžném procházení. `Parkoviště a brána`, Milesight ani Loxone Builder se v macOS Menu nezobrazují.
...
Launcher 3.0.0.16 jednou za minutu obnovuje diagnostiku podpisu skriptu, nalezených Configů, UI Automation, oprávnění a bezpečného spojení s Hubem. Po volbě v Evora Smart Menu buď použije již otevřený proces přesné verze, nebo ověřeně spustí nové okno; chybějící běžící verzi nikdy tiše nenahradí jiným procesem. Položka **LOXONE Config verze** v Menu zadá samostatnou úlohu právě tomuto Windows Launcheru: ten stáhne vybraný oficiální Stable, Beta nebo Alpha ZIP přímo do složky Stažené soubory ve Windows, ověří HTTPS doménu LOXONE, velikost i ZIP signaturu a teprve potom soubor zveřejní. macOS odkaz přímo nestahuje. V již otevřeném Configu primárně fyzicky klikne do znovu ověřených hranic prvku Domů, po každé Qt navigaci obnoví `MainWindowHandle`, rozpozná přesné české varianty `Ručně připojit` i `Manuálně připojit` a potom vyplní údaje a připojí. Běží skrytě v oznamovací oblasti Windows pod značkou Evora; přihlášenou relaci hlídá minutová uživatelská naplánovaná úloha spuštěná bez okna přes `wscript.exe` a pojmenovaný mutex brání duplicitám. Automatická aktualizace migruje i existující PowerShell watchdog a autostart bez nového párování. Adresu aktualizačního skriptu skládá za přesným Home Assistant ingress prefixem, ověří SHA-256, odstraní případný zděděný atribut jen pro čtení a oddělený proces přepíše hlavní skript až po skončení předchozího Launcheru. Náhradu přijme až po čerstvém autentizovaném heartbeat nové verze z přesné instalační cesty; při selhání vrátí `.bak`, znovu spustí předchozí verzi a ověří její připojení. Stavová tečka rozlišuje online, offline, aktualizaci a start. Z ikony lze otevřít Hub, ověřit připojení a aktualizace, spustit nové párování nebo otevřít diagnostický protokol. Před párováním samostatně ověří DNS, HTTPS certifikát a `/healthz`; `HUB_DNS_FAILED` znamená nedostupný překlad názvu ve Windows, nikoli chybný kód. `Opravit-parovani.cmd` dovolí změnit adresu Hubu a instalátor nenahradí funkční instalaci, dokud nové párování neprojde. Starý token zůstane platný až do prvního úspěšného heartbeat nového helperu. Přesný krok neúspěšné automatizace vrátí do Hubu bez loginu, hesla nebo tokenu a Config ponechá otevřený pro ruční připojení. Novou verzi přijme jen z manifestu vráceného po autentizovaném pollingu a po přesné shodě SHA-256. Tlačítko **Odebrat počítač** zneplatní token a čekající úlohy, ale lokální soubory ve Windows nemaže.
...
Registrace Entra musí být pouze pro účty daného organizačního tenantu, bez klientského tajemství a s povoleným přepínačem **Allow public client flows**. Endpoint Microsoft Graph `/shares` vyžaduje pro rozlišení položky ze sdíleného odkazu delegované `Files.ReadWrite`; širší `Files.ReadWrite.All` ani aplikační oprávnění se nepoužívají. Hub tuto relaci v 3.0.34 používá jen pro `GET` stažení sešitu. Do voleb Hubu se přenesou pouze Directory (tenant) ID a Application (client) ID. Pokud organizace zakazuje uživatelský souhlas, schválí stejné delegované oprávnění její správce. Bez obou identifikátorů a dokončeného device-code souhlasu se ruční i plánovaná synchronizace označí jako nenastavená a zachová poslední lokálně importovaná data.
...
Hub publikuje pouze kanál 7 `Parkoviště a brána`. Hub z přímého RTSP zdroje vytváří jedinou autentizovanou H.264/fMP4 HLS relaci; RTSP adresa ani přístupy se do browseru neposílají. Připnutý `init.mp4` umožní bezpečný start i klientovi otevřenému dlouho po vzniku relace. Playlist obsahuje jen souvislé fragmenty, které Hub už celé načetl a drží v omezené paměti; právě vznikající fragment se dokončí na pozadí a zveřejní teprve potom. Browser použije native HLS nebo připnutý hls.js fallback a stav `ŽIVĚ` vyžaduje opakovaný postup dekódovaného času; při zastavení zůstane poslední snapshot, zobrazí se chyba a spustí omezený reconnect. Aktivní přehrávání nepoužívá MJPEG. macOS Menu 3.0.31 kameru ani bránu nezobrazuje a při otevření nevytváří AVPlayer ani HLS prewarm.
